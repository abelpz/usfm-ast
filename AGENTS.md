...
3. **Before finishing:** run `bun install` (if needed), then `bun run lint`, `bun run check-types`, `bun run build`. For **tests**, run `CI=true bun run test` to match GitHub Actions (parser + adapters run full Jest except `parser.performance.*`; perf **budget** test runs in CI; see [`docs/09-ci-and-branch-protection.md`](./docs/09-ci-and-branch-protection.md)). Also run `bun run examples:check` when touching examples. After intentional **`USFMParser` / `toJSON()`** output changes, run `bun run regenerate:example-usj` and commit updated `examples/usfm-markers/**/example.usj`. Releases and logging: [`docs/16-production-readiness.md`](./docs/16-production-readiness.md), root [`CHANGELOG.md`](./CHANGELOG.md).
