import { resolveUSFMChrome } from '../src/chrome-resolve';
...
describe('resolveUSFMChrome', () => {
  ...
  it('accepts a custom theme string for data-usfm-theme', () => {
    const c = resolveUSFMChrome({ preset: 'default', theme: 'my-brand' });
    expect(c.theme).toBe('my-brand');
  });
});
