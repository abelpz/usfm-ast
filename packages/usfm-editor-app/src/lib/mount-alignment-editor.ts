/** Word-alignment UI mount (vanilla DOM). Styles: `legacy-panels.css`. */
...
export function mountAlignmentEditor(container: HTMLElement, session: ScriptureSession): () => void {
  ...
  function render() {
    ...
    const tgtGroupIndex = (ti: number): number | null => {
      for (let gi = 0; gi < groups.length; gi++) {
        ...
        for (const tw of t) {
          const idx = gwTokens.findIndex((w, wi) => w === tw.word && wi + 1 === ti + 1);
          if (idx === ti) return gi;
        }
      }
      for (let gi = 0; gi < groups.length; gi++) {
  ...
  return () => {
    ...
    unCh();
    container.innerHTML = '';
    container.classList.remove('alignment-editor');
  };
}
