...
function getParagraphMenuChoices(session: ScriptureSession, section: EditorSection): MarkerChoice[] {
  return session.markers.getChoicesForMode(section, readEditorMode());
}
...
function applyMarkerFromPalette(view: EditorView, id: string, session: ScriptureSession): void {
  ...
  if (id === SPECIAL_MARKER.SPLIT) {
    insertParagraph('p', { editorMode: readEditorMode() })(state, view.dispatch);
    return;
  }
  ...
  const em = { editorMode: readEditorMode() };
  if (info.inParagraph) {
    if (info.mode === 'empty') {
      changeParagraphMarker(id, em)(state, view.dispatch);
      return;
    }
    if (info.mode === 'mid') {
      insertParagraph(id, em)(state, view.dispatch);
      return;
    }
    if (info.mode === 'end' && info.blockStart !== null) {
      insertParagraphAfterBlock(info.blockStart, id, em)(state, view.dispatch);
      return;
    }
  }
  ...
  if (depth > 0) {
    ...
    insertParagraphAfterBlock(bs, id, { editorMode: readEditorMode() })(state, view.dispatch);
  }
}
...
function applyParagraphMarkerFromAddMenu(view: EditorView, marker: string, pendingBlockStart: number): void {
  ...
  if (!sameBlock || !info.inParagraph) {
    insertParagraphAfterBlock(pendingBlockStart, marker, em)(state, view.dispatch);
    return;
  }
  if (info.mode === 'empty') {
    changeParagraphMarker(marker, em)(state, view.dispatch);
    return;
  }
  if (info.mode === 'mid') {
    insertParagraph(marker, em)(state, view.dispatch);
    return;
  }
  insertParagraphAfterBlock(pendingBlockStart, marker, em)(state, view.dispatch);
}
...
interface BlockMenuEntry {
  ...
  /** Pre-rendered row icon (structural inserts) or {@link PresentationLayer.markerIcon} override. */
  menuGraphic?: string | HTMLElement;
  ariaLabel?: string;
}

function finalizeBlockMenuEntries(
  entries: BlockMenuEntry[],
  mode: EditorMode,
  pres: ResolvedPresentationLayer
): BlockMenuEntry[] {
  if (mode === 'advanced') return entries;
  return entries.map((ent) => {
    const structural = pres.structuralRow(ent.id);
    if (structural) {
      return {
        ...ent,
        label: structural.label,
        category: pres.menuCategory(ent.category, mode),
        menuGraphic: structural.icon,
    ...
    return {
      ...ent,
      category: pres.menuCategory(ent.category, mode),
      iconKey: docsIconKeyForParagraphMarker(ent.id),
      menuGraphic: undefined,
    };
  });
}
...
function buildAddMenuEntries(
  ...
  const struct = session.markers.getStructuralInsertions(state, pos);
  ...
  if (session.markers.canInsertVerse(section)) {
    out.push({
  ...
  if (session.markers.canInsertChapter(section) && session.canInsertNextChapter()) {
    out.push({
  ...
  for (const ch of getParagraphMenuChoices(session, section)) {
    out.push({ id: ch.marker, label: ch.label, category: ch.category });
  }
  return finalizeBlockMenuEntries(out, mode, pres);
}
...
function fillMenuRowButton(
  ...
  docsUi: boolean,
  ariaLabel: string,
  opts: { iconKey?: DocsMenuIconKey; menuGraphic?: string | HTMLElement; markerId?: string },
  pres: ResolvedPresentationLayer
): void {
  b.setAttribute('aria-label', ariaLabel);
  let graphicHtml: string | null = null;
  let graphicEl: HTMLElement | null = null;
  const { iconKey, menuGraphic, markerId } = opts;
  if (menuGraphic !== undefined) {
    if (typeof menuGraphic === 'string') graphicHtml = menuGraphic;
    else graphicEl = menuGraphic;
  } else if (markerId && docsUi) {
    const custom = pres.markerIcon(markerId);
    if (custom !== null && custom !== undefined) {
      if (typeof custom === 'string') graphicHtml = custom;
      else graphicEl = custom;
    }
  }
  const showIcon = docsUi && (graphicHtml !== null || graphicEl !== null || iconKey);
  if (showIcon) {
    ...
    if (graphicEl) icon.appendChild(graphicEl);
    else if (graphicHtml !== null) icon.innerHTML = graphicHtml;
    else if (iconKey) icon.innerHTML = menuIconSvg(iconKey);
...
export interface WysiwygChromeOptions {
  /** Owning session — marker lists and structure rules come from {@link ScriptureSession.markers}. */
  session: ScriptureSession;
  /** Optional presentation overrides (labels, icons, simplified-mode chrome). */
...
export function attachWysiwygChrome(
  ...
  const pres = mergePresentationLayer(
    createDocsPresentationLayer(session.markers),
  ...
  changeMenuHeading.textContent = pres.blockMenuTitle(readEditorMode());
  ...
  function wireBlockChoiceButtons(
    ...
    const mode = readEditorMode();
    const docsUi = pres.isSimplifiedMode(mode);
    container.replaceChildren();
    let lastCat: string | undefined;
    for (const ch of choices) {
      const cat = mode === 'advanced' ? ch.category : pres.menuCategory(ch.category, mode);
      if (cat && cat !== lastCat) {
        lastCat = cat;
      ...
      const iconKey = docsUi ? docsIconKeyForParagraphMarker(ch.marker) : undefined;
      const aria = ch.label;
      fillMenuRowButton(b, ch.label, docsUi, aria, { iconKey, markerId: ch.marker }, pres);
  ...
  function wireAddMenuEntries(entries: BlockMenuEntry[], container: HTMLElement, onPick: (id: string) => void) {
    ...
    for (const ent of entries) {
      ...
      fillMenuRowButton(b, ent.label, docsUi, aria, {
        iconKey: ent.iconKey,
        menuGraphic: ent.menuGraphic,
        markerId: ent.id,
      }, pres);
      b.addEventListener('click', (e) => {
        e.preventDefault();
  ...
  function rebuildPaletteList() {
    ...
    for (const ent of paletteFiltered) {
      ...
      b.addEventListener('click', (e) => {
        e.preventDefault();
        e.stopPropagation();
        applyMarkerFromPalette(view, ent.id, session);
        closePalette();
        view.focus();
      });
  ...
  function openMarkerPalette(v: EditorView) {
    ...
    const section = session.markers.getSectionAtPos(state, pos);
  ...
  function blockMenusForBlockStart(blockStart: number | null): void {
    ...
    const section = session.markers.getSectionAtPos(view.state, pos);
    changeMenuHeading.textContent = pres.blockMenuTitle(readEditorMode());
    const addEntries = buildAddMenuEntries(session, view.state, pos, section, pres);
    wireAddMenuEntries(addEntries, menu, (id) => {
      ...
      if (
        ...
        applyMarkerFromPalette(view, id, session);
      } else {
    ...
    const choices = getParagraphMenuChoices(session, section);
    wireBlockChoiceButtons(choices, changeMenuBody, (marker) => {
      if (pendingBlockStart === null) return;
      changeParagraphMarker(marker, { editorMode: readEditorMode() })(view.state, view.dispatch);
      view.focus();
      changeMenu.hidden = true;
    });
  ...
  /** Delay before the bubble becomes visible after the cursor settles (ms). */
  const BUBBLE_SHOW_DELAY_MS = 600;
  let bubbleShowTimer: ReturnType<typeof setTimeout> | null = null;
  ...
  /**
   * Schedule `updateBubble` to run after {@link BUBBLE_SHOW_DELAY_MS}.
   * Cancels any pending timer so rapid cursor moves reset the clock.
   */
  ...
  function applyBubbleGraphic(el: HTMLElement, graphic: string | HTMLElement) {
    el.replaceChildren();
    if (typeof graphic === 'string') el.innerHTML = graphic;
    else el.appendChild(graphic);
  }
    ...
    for (const a of actions) {
      ...
      if (a.toolbarIcon) {
        ...
        if (hasLabel) {
          ...
          applyBubbleGraphic(ic, pres.bubbleIcon(a.toolbarIcon));
          const lab = document.createElement('span');
          lab.className = 'usfm-wysiwyg-bubble-label';
          lab.textContent = a.label;
          btn.append(ic, lab);
        } else {
          applyBubbleGraphic(btn, pres.bubbleIcon(a.toolbarIcon));
        }
  ...
  function positionGutterForEl(
    ...
    const mode = readEditorMode();
    markerBtn.classList.toggle('usfm-wysiwyg-marker-chip--docs', pres.isSimplifiedMode(mode));
    if (currentMarker) {
      let label: string;
      if (blockStart !== null) {
        const pos = Math.min(blockStart + 1, view.state.doc.content.size);
        const section = session.markers.getSectionAtPos(view.state, pos);
        label = pres.markerLabel(currentMarker, section, mode);
      } else {
        label = mode === 'advanced' ? `\\${currentMarker}` : currentMarker;
      }
      if (pres.isSimplifiedMode(mode)) {
        markerBtn.replaceChildren();
        const iconWrap = document.createElement('span');
        iconWrap.className = 'usfm-wysiwyg-marker-chip-icon';
        iconWrap.setAttribute('aria-hidden', 'true');
        applyBubbleGraphic(iconWrap, pres.gutterBlockOptionsIconSvg());
    ...
    // Prefer placing the strip just left of the block (marker chip, then +).
    const gap = 4;
    const edgePad = 8;
    const stripW = gutterStrip.offsetWidth || 60;
    const win = view.dom.ownerDocument.defaultView ?? window;
    const vw = win.visualViewport?.width ?? win.innerWidth;
    let left = r.left - stripW - gap;
    // When the paragraph is near the viewport (or card) edge, the naive left is
    // negative and clips the marker chip / + off-screen — clamp so both stay visible.
    const maxLeft = Math.max(edgePad, vw - stripW - edgePad);
    left = Math.max(edgePad, Math.min(left, maxLeft));
    gutterStrip.style.left = `${Math.round(left)}px`;
  }
  ...
  function hideChromeForScroll(e?: Event) {
    ...
    scrollEndTimer = setTimeout(() => {
      scrollEndTimer = null;
      scrolling = false;
      if (bubbleEngaged) scheduleBubble();
    }, 150);
  }
  ...
  function onSelectionChange() {
    requestAnimationFrame(() => {
      ...
      // Hide immediately when selection moves; idle timer will re-show.
      concealBubble();
      scheduleBubble();
    });
  }
  ...
  function onEditorKeyup(e: KeyboardEvent) {
    ...
    if (bubbleEngaged) {
      // Hide immediately on any key activity; show again only after idle.
      concealBubble();
      scheduleBubble();
    }
  }
