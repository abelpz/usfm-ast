...
describe('USXVisitor converts USFM to USX correctly', () => {
  ...
  const testVisitorConversionFromFiles =
    (inputPath: [string, string], expectedPath: [string, string]) => async () => {
      ...
      await testVisitorConversionFromStrings(
        { provided, expected },
        { outputDocument: true, beautify: true }
      )();
    };

  const testVisitorConversionFromStrings =
    (
      { provided, expected }: { provided: string; expected: string },
      {
        outputDocument = false,
        beautify = false,
      }: { outputDocument?: boolean; beautify?: boolean } = {}
    ) =>
    async () => {
      ...
      expect(JSON.stringify(parsedActual, null, 2)).toEqual(
  ...
  describe('Outputs correct USX structure', () => {
    test(
      ...
      testVisitorConversionFromFiles(['usfm', 'rev.lsg.usfm'], ['usx', 'rev.lsg.xml'])
    );

    test(
      `converts structure/verses.usfm to expected USX output`,
      testVisitorConversionFromFiles(['structure', 'verses.usfm'], ['structure', 'verses.xml'])
    );

    test(
      `converts structure/milestones.usfm to expected USX output`,
    ...
    test(
      `converts full document to xml`,
      testVisitorConversionFromFiles(['usfm', 'tit.bsb.usfm'], ['usx', 'tit.bsb.xml'])
    );
  });
  ...
  describe('converts markers to USX correctly', () => {
    test(
      ...
      testVisitorConversionFromStrings({
